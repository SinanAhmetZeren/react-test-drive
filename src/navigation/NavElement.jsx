...
function NavElement() {
  const [paramsDegeri, setParamsDegeri] = useState(1);
  return (
    <div>
      <ul className="flex flex-wrap items-center justify-center ">
        ...
        <li className="mt-0.5">
          <NavLink
            ...
            to={`/params/${paramsDegeri}`}
          >
            Params
          </NavLink>

          <input
            className="w-30 h-3 bg-yellow-200 rounded-lg appearance-none cursor-pointer"
            type="range"
            min="1"
            max="3"
            value={paramsDegeri}
            onChange={(e) => setParamsDegeri(Number(e.target.value))}
          />
          <span>{paramsDegeri}</span>
        </li>
      </ul>
    </div>
  );
